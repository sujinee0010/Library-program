...
public class Library extends JFrame implements ActionListener {
	...
	//필드
	public JTextField txtURL,txtTitle,txtKind,txtAuthor,txtPublisher,txtPrice, txtISBN;
	public JButton btnSearch, btnLoad;
	public JButton btnFirst, btnPrevious, btnNext, btnLast;
	public JButton btnNew, btnFind, btnModify, btnAdd, btnDelete, btnSave, btnClose;

	public Document xmlDoc;
	public Element ePrevious, eCurrent, eNext;
	public boolean isAddReady, isFindReady, isDeleteReady;	
	...
	public Library() {
		...
		JLabel pNorthlblNorth = new JLabel("도서 검색");
		...
		JPanel pCenterpCenterpWest = new JPanel(new GridLayout(6,1,10,10));
		...
		pCenterpCenterpWest.add(lblISBN);
			
		JPanel pCenterpCenterpCenter = new JPanel(new GridLayout(6,1,10,10));
		...
		pCenterpCenterpCenter.add(txtISBN);
		...
		JPanel pEastpCenter = new JPanel(new GridLayout(6,1,0,20));	
		...
		pEastpCenter.add(btnFind);
		pEastpCenter.add(btnModify);
		pEastpCenter.add(btnAdd);
		pEastpCenter.add(btnDelete);
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if(obj==btnFind)			btnFind_Click();
		if(obj==btnModify)			btnModify_Click();
		if(obj==btnAdd)				btnAdd_Click();
		if(obj==btnDelete)			btnDelete_Click();
	...
	public void btnLoad_Click() {
		try {
			...
			//3.xml  문서 파싱 
		//	Document document = parser.parse("booklist.xml");
			//4.
			// 원격 파일인지 조사
...
//			String strTitle = eTitle.getTextContent();
			...
			if(strTitle.contains(txtTitle.getText())) {			
				// 모든 엘리먼트와 속성값 디스플레이 
				displayBook(eBook);					
		  }
	}				
	...
	public void btnModify_Click() {
		...
		//ISBN 엘리먼트의 컨텐트 내용 수정
		Element eISBN = (Element)ePrice.getNextSibling();
		Text tISBN = (Text)eISBN.getFirstChild();
		tISBN.setData(txtISBN.getText());	
	...
	public void btnAdd_Click() {
    	...
    	//ISBN 엘리먼트 객체 생성
    	Element eISBN = xmlDoc.createElement("ISBN");
    	Text tISBN = xmlDoc.createTextNode(txtISBN.getText());
    	eISBN.appendChild(tISBN);
	...
	public void displayBook(Element eBook) {	
	    ...
	    //ISBN 엘리먼트의 컨텐트 내용을 얻어내어 디스플레이
	    Element eISBN = (Element)ePrice.getNextSibling();
	    Text tISBN = (Text)eISBN.getFirstChild();
	    txtISBN.setText(tISBN.getData());		
	}
